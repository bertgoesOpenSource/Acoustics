 % Copyright (C) <2010>  <Chen (Bert) Jiang>
 % umjiang@umich.edu
...
function varargout = GUI_single(varargin)
...
% Last Modified by GUIDE v2.5 29-Nov-2010 00:15:14

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_single_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_single_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function GUI_single_OpeningFcn(hObject, eventdata, handles, varargin)
...
Rate = 10000; %Set sample rate here
...
function varargout = GUI_single_OutputFcn(hObject, eventdata, handles) 
...
%       
        ...
        subplot(4,1,1)%,'Position',[0.1,0.8,0.9,0.1])

        ft = fft(Receiver((start*Rate+1):ending*Rate,1),NFFT); %
			
        plot(time(round(start*Rate+1):round(ending*Rate)),...
            Receiver(round(start*Rate+1):round(ending*Rate),1));
         xlim([start ending])
		
         %   ylim([-5e-3,5e-3]);
         subplot(4,1,2)%,'Position',[.1,.65,.8,.1])
		...
		%TODO fix source
			  %	----------------%Plot Source Signal
		source_offset = 0; %Source delay in sec
		
%debug

		 subplot(4,1,3)%,'Position',[0.1,0.4,0.8,0.2])
        plot(time_Source(round((start)*Source_Rate+1):round((ending)*Source_Rate))...
        ,Source(round((start-offsetVal)*Source_Rate+1):round((ending-offsetVal)*Source_Rate)));
...
%         round(ending*Rate)
    ...
    set(handles.text7,'String',(start-offsetVal)*Source_Rate+1);    
    set(handles.text8,'String',(ending-offsetVal)*Source_Rate+1);   
end
...
function enableFFT_DeleteFcn(hObject, eventdata, handles)
% hObject    handle to enableFFT (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
